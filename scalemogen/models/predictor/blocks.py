"""Transformer blocks used by the ScaleMoGen predictor.

Code provenance: adapted from the Infinity autoregressive transformer and
reduced to the standard PyTorch attention path used by ScaleMoGen.
...
def precompute_rope2d_freqs_grid(dim, scale_schedule, rope2d_normalized_by_hw, pad_to_multiplier=1, max_height=2048 // 16, max_width=2048 // 16, base=10000.0, device=None, scaling_factor=1.0):
    ...
    rope2d_freqs_grid[str(tuple(scale_schedule))] = cat_rope_cache
...
class FastRMSNorm(nn.Module):
    ...
    def forward(self, x):
        src_type = x.dtype
        x = x.float()
        return (x * torch.rsqrt(x.pow(2).mean(-1, keepdim=True).add_(self.eps)) * self.weight).to(src_type)
...
class FFN(nn.Module):
    ...
    def extra_repr(self) -> str:
        return f'in_features={self.fc1.in_features}, hidden_features={self.fc1.out_features}'
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        proj_drop=0., tau=1, cos_attn=False, pad_to_multiplier=1, rope2d_normalized_by_hw=0,
    ):
        """
    ...
    def forward(self, x, attn_bias_or_two_vector: Optional[torch.Tensor], scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):
        """
        :param (fp32) x: shaped (B or batch_size, L or seq_length, C or hidden_dim); if seq-parallel is used, the `L` dim would be shared
        :param attn_bias_or_two_vector: additive attention mask where 0 means visible and -inf means invisible.
        ...
        # Keep the active ScaleMoGen path: PyTorch SDPA with an additive block-causal mask.
        oup = slow_attn(query=q, key=k, value=v, attn_mask=attn_bias_or_two_vector, dropout_p=0).transpose(1, 2).reshape(B, L, C)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, ca_kv):
        ...
        lengths = (cu_seqlens_k[1:] - cu_seqlens_k[:-1]).to(torch.long)
        B = int(lengths.numel())
        N = kv_compact.shape[0]

        kv_compact = F.linear(kv_compact, weight=self.mat_kv.weight, bias=torch.cat((self.zero_k_bias, self.v_bias))).view(N, 2, self.num_heads, self.head_dim)
        kv_padded = kv_compact.new_zeros(B, max_seqlen_k, 2, self.num_heads, self.head_dim)
        for batch_idx in range(B):
            start = int(cu_seqlens_k[batch_idx].item())
            end = int(cu_seqlens_k[batch_idx + 1].item())
            kv_padded[batch_idx, : end - start] = kv_compact[start:end]

        k, v = kv_padded.unbind(dim=2)
        k = k.permute(0, 2, 1, 3).contiguous()
        v = v.permute(0, 2, 1, 3).contiguous()

        if not self.for_attn_pool:
            Bq, Lq = q.shape[:2]
            assert Bq == B, f"query batch size {Bq} does not match text batch size {B}"
            q_compact = self.mat_q(q).view(B, Lq, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
        else:
            Lq = 1
            q_compact = self.mat_q.expand(B, -1, -1).unsqueeze(2).to(dtype=kv_compact.dtype)
        ...
        valid_k = torch.arange(max_seqlen_k, device=kv_compact.device).unsqueeze(0) < lengths.to(kv_compact.device).unsqueeze(1)
        attn_bias = kv_compact.new_zeros(B, 1, 1, max_seqlen_k)
        attn_bias = attn_bias.masked_fill(~valid_k[:, None, None, :], torch.finfo(attn_bias.dtype).min)
        oup = slow_attn(q_compact.contiguous(), k, v, attn_mask=attn_bias, dropout_p=0).transpose(1, 2).reshape(B, Lq, -1)
...
class SelfAttnBlock(nn.Module):
    def __init__(
        ...
        swiglu=False, checkpointing_sa_only=False,
    ):
        ...
        self.ffn = (FFNSwiGLU if swiglu else FFN)(in_features=embed_dim, hidden_features=round(embed_dim * mlp_ratio / 256) * 256, drop=drop)
    ...
    # NOTE: attn_bias_or_two_vector is None during inference
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):  # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        x = x + self.drop_path(self.ffn(self.ln_wo_grad(x.float()).mul(scale2.add(1)).add_(shift2)).mul(gamma2))
        return x
...
class CrossAttnBlock(nn.Module):
    def __init__(
        ...
        num_heads, mlp_ratio=4., drop=0., drop_path=0., tau=1, cos_attn=False,
        swiglu=False, checkpointing_sa_only=False,
        pad_to_multiplier=1, apply_rope2d=False, rope2d_normalized_by_hw=False,
    ):
    ...
    def forward(self, x, cond_BD, ca_kv, attn_bias_or_two_vector, scale_schedule=None, rope2d_freqs_grid=None, scale_ind=0):    # todo: minGPT and vqgan also uses pre-norm, just like this, while MaskGiT uses post-norm
        ...
        if self.checkpointing_sa_only and self.training:
            x_sa = checkpoint(self.sa, x_sa, attn_bias_or_two_vector, scale_schedule, rope2d_freqs_grid, scale_ind, use_reentrant=False)
        else:
            x_sa = self.sa(x_sa, attn_bias_or_two_vector, scale_schedule, rope2d_freqs_grid, scale_ind=scale_ind)
    ...
    def extra_repr(self) -> str:
        return f'shared_aln={self.shared_aln}, ca_gamma={"<learnable>" if isinstance(self.ca_gamma, nn.Parameter) else self.ca_gamma}'


class AdaLNBeforeHead(nn.Module):
    def __init__(self, C, D, act: bool, norm_layer: partial):   # C: embed_dim, D: cond_dim
        super().__init__()
        self.C, self.D = C, D
