"""Miscellaneous runtime helpers for ScaleMoGen predictor training.

Code provenance: adapted from the Infinity training utilities.
...
from scalemogen.utils.console import METRIC_ALIASES
...
def os_system_get_stdout_stderr(cmd):
    ...
    while True:
        try:
            ...
            cnt += 1
            print(f"[ScaleMoGen][Runtime] command_timeout count={cnt}", flush=True)
        else:
...
class TensorboardLogger(object):
    ...
    def log_tensor_as_distri(self, tag, tensor1d, step=None):
        ...
        try:
            ...
            print(f"[ScaleMoGen][TensorBoard] histogram_failed={tag}: {e}", flush=True)
...
class TouchingDaemonDontForgetToStartMe(threading.Thread):
    ...
    def run(self) -> None:
        ...
        if self.verbose:
            print(
                f"[ScaleMoGen][Runtime] touch_daemon=start tid={threading.get_native_id()} "
                f"files={self.files} interval={self.sleep_secs}s",
                flush=True,
            )
        ...
        if self.verbose:
            print(
                f"[ScaleMoGen][Runtime] touch_daemon=finish tid={threading.get_native_id()} "
                f"elapsed={time.time() - stt:.1f}s files={self.files}",
                flush=True,
            )
...
class MetricLogger(object):
    ...
    def __str__(self):
        ...
        for name, meter in self.meters.items():
            if len(meter.deque):
                loss_str.append(
                    "{}={}".format(METRIC_ALIASES.get(name, name), str(meter))
                )
    ...
    def log_every(self, start_it, max_iters, itrt, log_freq, log_every_iter=False, header=''):    # also solve logging & skipping iterations before start_it
        ...
        if isinstance(itrt, Iterator) and not hasattr(itrt, 'preload') and not hasattr(itrt, 'set_epoch'):
            for it in range(start_it, max_iters):
                ...
                if self.log_every_iter or it in self.log_iters:
                    ...
                    print(
                        f'{header} it={it + 1:0{width}d}/{max_iters} '
                        f'eta={str(datetime.timedelta(seconds=int(eta_seconds)))} '
                        f'{str(self)} step={self.iter_time.value:.3f}s data={self.data_time.value*1e3:.1f}ms',
                        flush=True,
                    )
                self.iter_end_t = time.time()
        else:
        ...
        print(f'{header} done elapsed={cost_str} avg_step={cost / (max_iters-start_it):.3f}s', flush=True)
...
def build_2d_sincos_position_embedding(h, w, embed_dim, temperature=10000., sc=0, verbose=True):    # (1, hw**2, embed_dim)
    ...
    if verbose:
        print(
            f"[ScaleMoGen][Runtime] sincos_pos_embed hw={hw} scale_type={sc} "
            f"temperature={temperature:g} shape={pos_emb.shape}",
            flush=True,
        )
...
if __name__ == '__main__':
    ...
    for row, sc in enumerate(scs):
        for col, temp in enumerate(temps):
            ...
            print(f"[ScaleMoGen][Runtime] tensor_distance name={name} distance={dis}", flush=True)
            """
